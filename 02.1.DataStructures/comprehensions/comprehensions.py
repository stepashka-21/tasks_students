...
def get_unique_page_ids_visited_after_ts(records: list[tp.Mapping[str, tp.Any]], ts: int) -> set[int]:
    ...
    """
    return {record['PageID'] for record in records if record.get('EventTime', 0) > ts}
...
def get_unique_user_ids_visited_page_after_ts(
    ...
    """
    return {record['UserID'] for record in records if
            record.get('EventTime', 0) > ts and record.get('PageID') == page_id}
...
def get_region_ids_with_none_replaces_by_default(
    ...
    :return: region ids
    """
    return [record.get('RegionID', DEFAULT_REGION_ID) if record.get('RegionID') is not None else DEFAULT_REGION_ID for record in records]
...
def get_region_id_if_not_none(
    ...
    :return: region ids
    """
    return [record['RegionID'] for record in records if 'RegionID' in record and record['RegionID'] is not None]
